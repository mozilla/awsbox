...
exports.installPackages = function(host, packages, cb) {
  if (!packages || !packages.length) cb();
    ...
    var options = { maxBuffer: 5000 * 1024 };
    var cp = child_process.execFile(ssh, args, options, function(err) {
      if (err) return cb(err);
      var rcmd = "echo '" + pkg + "' >> packages.txt";
      var args = ['-o', 'StrictHostKeyChecking no', destination, rcmd];
      child_process.execFile(ssh, args, options, function(err) {
        if (err) return cb(err);
...
exports.updatePackages = function(host, cb) {
  ...
  var cp = child_process.execFile(ssh, args, options, function(err) {
    ...
    var args = ['-o', 'StrictHostKeyChecking no', destination, rcmd];
    cp = child_process.execFile(ssh, args, options, function(err) {
      if (err) return cb(err);
      cb();
    });
    passthrough(cp);
