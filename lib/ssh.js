...
exports.copyUpConfig = function(host, config, cb) {
  ...
  temp.open({}, function(err, r) {
    ...
    function oneTry() {
      child_process.execFile(scp, args, function(err, r) {
        if (err) {
...
exports.installPackages = function(host, packages, cb) {
  if (!packages || !packages.length) cb();
	...
	var args = ['-o', 'StrictHostKeyChecking no', destination, rcmd];
    var cp = child_process.execFile(ssh, args, function(err, r) {
      if (err) return cb(err);
	  var rcmd = "echo '" + pkg + "' >> packages.txt";
	  var args = ['-o', 'StrictHostKeyChecking no', destination, rcmd];
      child_process.execFile(ssh, args, function(err, r) {
        if (err) return cb(err);
...
exports.updatePackages = function(host, cb) {
  ...
  var cp = child_process.execFile(ssh, args, function(err, r) {
    ...
    var args = ['-o', 'StrictHostKeyChecking no', destination, rcmd];
    cp = child_process.execFile(ssh, args, function(err, r) {
      if (err) return cb(err);
      cb();
    });
    passthrough(cp);
...
exports.addSSHPubKey = function(host, pubkey, cb) {
  ...
  var destination = 'ec2-user@' + host;
  var rcmd = "grep '" + pubkey + "' .ssh/authorized_keys || echo '" + pubkey + "' >> .ssh/authorized_keys'";
  var args = ['-o', 'StrictHostKeyChecking no', destination, rcmd];
  child_process.execFile(ssh, args, cb);
};
