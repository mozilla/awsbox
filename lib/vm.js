...
exports.destroy = function(name, cb) {
  exports.list(function(err, r) {
    ...
    ec2.TerminateInstances({
      ...
      // ToFix: ... check where the error message is
      if (err) return cb(err.Errors.Error.Message.something.something);
      cb(null, deets);
    });
  });
};
...
exports.waitForInstance = function(id, cb) {
  ec2.DescribeInstanceStatus({
    ...
    // ToDo: check if we never get a response back (or is that a function of aws-lib?)
    ...
    if (Object.keys(r.instanceStatusSet).length) {
      ...
      if (deets && deets.instanceState && deets.instanceState.name === 'running') {
        return ec2.DescribeInstances( { InstanceId: id }, function(err, result) {
          returnSingleImageInfo(err, result, cb);
        });
      }
    }
