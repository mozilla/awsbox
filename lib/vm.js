...
const TEMPLATE_IMAGE_ID = 'ami-4b1c5722';
      ...
      if (!err) {
        ...
        try {
          if (err.Body.Response.Errors.Error.Message) {
            ...
            e = err.Body.Response.Errors.Error.Message;
          }
        } catch(ex) { };
...
exports.setName = function(id, name, cb) {
  aws.client.CreateTags({
    ...
    if (err) return cb(err.Body.Response.Errors.Error.Message);
    // ToDo: check if there are ever any errors which aren't in err.Body.Response.Errors.Error.Message???
    cb();
  });
};
